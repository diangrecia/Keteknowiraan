...
import shutil
from django.conf import settings
from .CreateImageModule import CreateImageMotif, _get_cached_image_data
...
def _run_single_seed(base_args, seed, is_even):
    generator = CreateImageMotif(*base_args, seed=seed)
    
    if is_even:
        url, urutan = generator.imageEven()
    else:
        url, urutan = generator.imageOdd()
        
    return seed, url, urutan
...
def external(request):
    ...
    # ── Validasi format, jumlah baris, dan spesifikasi gambar ────────────
    ...
    if formatStatus == "0":
        ...
        return render(request, 'home.html', {
            "jmlBaris": jmlBaris, "status": None,
            'navlink1': navlink[0], 'navlink2': navlink[1],
            'navlink3': navlink[2], 'navlink4': navlink[3]
        })
    ...
    # ── Setup session dan base args ───────────────────────────────────────
    session_name = f"session_{uuid.uuid4()}"
    base_args    = (str(fileurl), str(filename), str(jmlBaris), Baris, "4", username, session_name)
 
    # ── imageOriginal (tidak perlu diparallelkan, hanya 1x) ──────────────
    Image0 = CreateImageMotif(*base_args)
    UrutanLidiRaw, UrutanLidiIndex, url_raw_path = Image0.imageOriginal()
    url_raw_path = [f"/media/{p}" for p in url_raw_path]
 
    # ── OPTIMASI: Pre-load cache SEBELUM thread dijalankan ───────────────
    # Semua 4 thread akan menggunakan gambar yang sama.
    # Dengan pre-load di sini, imread + ConvertRGB_from_array +
    # ConvertArrayImage + permutations hanya dihitung SEKALI,
    # lalu ke-4 thread langsung cache hit.
    _get_cached_image_data(str(fileurl))
    print("[VIEWS] Cache pre-loaded, memulai parallel processing...")
 
    # ── OPTIMASI: Jalankan 4 seed PARALLEL dengan ThreadPoolExecutor ─────
    # Sebelum: 4x sequential ~45 detik → total ~180 detik
    # Sesudah: 4x parallel   ~45 detik → total ~45 detik (waktu terlama)
    #
    # max_workers=4 karena ada tepat 4 task.
    # Thread aman digunakan di sini karena setiap thread:
    #   - Membuat objek CreateImageMotif sendiri (tidak share state)
    #   - Hanya membaca dari cache (tidak menulis, kecuali saat miss)
    #   - Menyimpan ke path unik (uuid) sehingga tidak ada konflik file
    is_even  = (jmlBaris % 2 == 0)
    results  = {}   # {seed: (url, urutan)}
 
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {
            executor.submit(_run_single_seed, base_args, seed, is_even): seed
            for seed in range(4)
        }
        for future in as_completed(futures):
            seed, url, urutan = future.result()
            results[seed] = (url, urutan)
 
    # Ambil hasil berdasarkan urutan seed 0-3
    URLEdit,  UrutanLidi  = results[0]
    URLEdit2, UrutanLidi2 = results[1]
    URLEdit3, UrutanLidi3 = results[2]
    URLEdit4, UrutanLidi4 = results[3]
 
    print(f"UrutanLidiRaw: {UrutanLidiRaw}")
 
    # ── Konversi semua urutan ke int ──────────────────────────────────────
...
#@login_required(login_url='login')
@csrf_exempt
def show(request): #untuk Daftar yang ada di fitur motif"
    # user = request.user
    # status = user.is_staff
    navlink = ['nav-link nav-link-1 ', 'nav-link nav-link-2', 'nav-link nav-link-3 active', 'nav-link nav-link-4']
    status = 1
...
def coloring_view(request):
    ...
    if request.method == 'GET':
        ...
        selected_ulos_type = selected_ulos_type.lower()
        selected_motif_id = selected_motif_id.lower()
